...
export class ProductDetailComponent implements OnInit {
  ...
  public updateProduct(): void {
    this.navigateToPage(`cadastro/produto/atualizar?product=${this.product.productId}`);
  }
  ...
  private getProduct(): void {
    ...
    this.management = management && this.authService.getAuthenticationState().roleId === Roles.STORE_ADMIN;
    ...
    if (productId && productId !== NaN) {
      ...
      this.productService.getProductById(productId).subscribe(response => {
        if (response.success) {
          ...
          if (!management) {
            this.storeService.getStoreById(this.product.storeId).subscribe(response => {
